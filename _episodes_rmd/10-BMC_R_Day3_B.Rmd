...
We have previously shown you how to wrangle your data into the right shape with the tidyverse. Now we will use core R librarie to perform hypothesis testing and statistical analysis. 
...
> > ggplot(gait, aes(y=TUAG, x = Group)) + geom_violin() +theme_bw()
> > ~~~
> > {: .output}
> {: .solution}
{: .challenge}
...
The Mann–Whitney U test, also called the Mann–Whitney–Wilcoxon, Wilcoxon rank-sum test, or Wilcoxon–Mann–Whitney test (uggh) is sometimes preferred to the 2 sample t-test when the distribution cannot be assumed to be normal; for a discussion of whether this is appropriate see [here](http://thestatsgeek.com/2014/04/12/is-the-wilcoxon-mann-whitney-test-a-good-non-parametric-alternative-to-the-t-test/). 
...
For our dummy example, we can perform an ANOVA using the `Study` grouping of patients. Here, we are testing the null hypothesis that the mean `TUAG` is the same for all groups.

```{r, warning=FALSE, message = FALSE}
aov_study <- aov(TUAG ~ Study, data = gait) # change this to a 2-way anova using Study + something
